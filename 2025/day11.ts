...
type ParseType = Record<string, E.Either<number, Array<string>>>;
...
async function readAndParseData(filePath: string): Promise<ParseType> {
  try {
    ...
    return pipe(
      lines,
      A.map((line): [string, E.Either<number, Array<string>>] => {
        const splLine = line.trim().split(": ");
        if (splLine.length !== 2) throw "parsing error";
...
const updateRec =
  (current: string, val: number) =>
  (conns: ParseType): ParseType =>
    pipe(
      conns,
      R.updateAt(current, E.left(val)),
      O.match(() => conns, identity)
    );

const DFS =
  (current: string, goal: string) =>
  (connections: ParseType): { conns: ParseType; paths: number } =>
    current === goal
      ? {
      ...
      : pipe(
          ...
          R.lookup(current),
          O.match(
            () => ({ conns: connections, paths: 0 }),
            (res) =>
              E.isLeft(res)
                ? { conns: connections, paths: res.left }
                : pipe(
                    res.right,
                    A.reduce({ conns: connections, paths: 0 }, (acc, neigh) =>
                      pipe(acc.conns, DFS(neigh, goal), (res) => ({
                        conns: res.conns,
...
const partA: PartType = flow(DFS("you", "out"), (result) => result.paths);

// --- Part B Logic ---

const partB = (parsed: ParseType): number =>
  DFS("svr", "fft")(parsed).paths *
