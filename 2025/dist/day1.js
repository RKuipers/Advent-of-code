...
async function readAndParseData(filePath) {
    try {
        ...
        for (const line of lines) {
            const l = line.trim();
            if (l[0] === "R")
                list.push(parseInt(l.slice(1)));
            else
                list.push(-1 * parseInt(l.slice(1)));
        }
        return list;
    }
    catch (error) {
...
// --- Part A Logic ---
const partA = flow(A.reduce({ count: 0, current: 50 }, (acc, v) => {
    const next = (acc.current + v) % 100;
    return { count: next === 0 ? acc.count + 1 : acc.count, current: next };
}), ({ count }) => count);
// --- Part B Logic ---
const partB = flow(A.reduce({ count: 0, current: 50 }, (acc, v) => {
    const fullTurns = Math.floor(Math.abs(v) / 100);
    const rem = v % 100;
    const next = acc.current + rem;
    const newCount = fullTurns + ((next <= 0 && acc.current !== 0) || next >= 100 ? 1 : 0);
    return {
