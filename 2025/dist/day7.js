...
async function readAndParseData(filePath) {
    try {
        ...
        // UPDATE FROM HERE
        const start = pipe(lines, A.head, O.flatMap((line) => pipe(line.trim().split(""), A.findIndex((c) => c === "S"))), O.match(() => 0, (x) => x));
        const splits = pipe(lines, A.dropLeft(1), A.map((line) => pipe(line.trim().split(""), A.reduceWithIndex([], (y, acc, next) => next === "^" ? [...acc, y] : acc))));
        return { splits, start };
        // UPDATE UNTIL HERE
    }
    catch (error) {
