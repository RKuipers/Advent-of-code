...
type Symbol = "+" | "*";
type ParseType = Array<[Array<number>, Symbol]>;
type PartType = (parsed: ParseType) => number;
const dayNumber = 6;

const transpose = <A>(list: Array<Array<A>>) =>
  pipe(
    list,
    A.reduce(
      A.replicate((list[0] ?? []).length, []),
      (acc: Array<Array<A>>, next: Array<A>) =>
        pipe(
          A.zip(acc, next),
          A.map(([a, n]) => [...a, n])
        )
...
async function readAndParseDataA(filePath: string): Promise<ParseType> {
  try {
    ...
    const symbols = pipe(
      ...
      A.map((char): Symbol => (char === "*" ? "*" : "+"))
    );

    const result = pipe(
      lines,
      A.dropRight(1),
      A.map((line) => line.trim().split(/\s+/)),
      transpose,
      A.map(A.map((entry) => parseInt(entry.trim()))),
      A.zip(symbols)
    );
...
async function readAndParseDataB(filePath: string): Promise<ParseType> {
  try {
    const data = await fs.readFile(path.resolve(filePath), "utf8");
    const lines = data.split("\n").filter((line) => line.trim() !== ""); // Read and filter empty lines
    ...
    const result = pipe(
      ...
      transpose,
      A.reverse,
      A.map((col) => parseInt(col.join(""))),
      A.reduce([], (acc: Array<Array<number>>, next) =>
        isNaN(next)
          ? [...acc, []]
          : [
              ...A.dropRight(1)(acc),
...
const partAB: PartType = flow(
  A.map(([numbers, symbol]) =>
    pipe(numbers, concatAll(symbol === "+" ? N.MonoidSum : N.MonoidProduct))
  ),
